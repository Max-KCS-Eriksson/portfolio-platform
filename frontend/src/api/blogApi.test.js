...
import { getBlogContext, getBlogPost, getBlogPosts, getBlogPostsByTag } from "./blogApi";
...
describe("blogApi", () => {
  ...
  test("loads and maps blog context", async () => {
    apiGet.mockResolvedValue({ id: 4, intro: "Blog intro" });

    await expect(getBlogContext()).resolves.toEqual({
      id: 4,
      intro: "Blog intro",
    });

    expect(apiGet).toHaveBeenCalledWith("/blog/context/");
  });
});
